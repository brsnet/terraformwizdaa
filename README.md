...
- `realtor-apps/manifests/deployment.yaml` and `service.yaml` define the actual `leads` Deployment (image `leads-leads:latest`, app listens on container port `6000`, set via the `PORT` env var in `leads-env`) and a `LoadBalancer` Service always exposing it at `http://localhost:7001` externally, proxied to the container's port `6000` (same Docker Desktop `LoadBalancer` behavior used for the Argo CD UI). External port `7001` is deliberately different from the app's internal `6000`: browsers (Chrome, etc.) hard-block port `6000` for HTTP as a legacy X11-security measure (`ERR_UNSAFE_PORT`), so the external LoadBalancer port must be a browser-safe one even though the app itself listens on `6000`.
